...
class ContrastiveNERTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # 即使 contrastive_weight 为 0，公式依然成立，相当于只用了基础 loss
...
output_dir = f"./outputs/results_wo_Contrastive_{current_time}"
...
trainer = ContrastiveNERTrainer(
    ...
    contrastive_weight=0.0  # ✂️ 【消融实验4】：切断对比学习损失 (w/o Semantic Contrastive)
)

print(f"🚀 消融实验四：去除对比学习 (w/o Contrastive)。结果保存在: {output_dir}")
trainer.train()

print("📊 训练结束。正在【测试集】上提取去除对比学习后的成绩...")
...
with open(os.path.join(output_dir, "evaluation_metrics.txt"), "w", encoding="utf-8") as f:
    f.write("========== w/o Contrastive Test Set Evaluation ==========\n")
...
print(f"🎉 实验四执行完毕！请查看控制台打印的 test_results，并记录 F1 分数！")