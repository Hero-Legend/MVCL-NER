...
# ==========================================
# 2. 数据处理与双视图特征提取 (保留处理逻辑以兼容DataLoader，但模型中不使用)
# ==========================================
...
class MVCL_BERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, lexical_ids, structural_ids, labels=None):
        ...
        # ✂️ 【消融实验1修改】：不融合词法(lex_embeds)和句法(struct_embeds)特征，使用纯底座
...
output_dir = f"./outputs/results_BASELINE_RoBERTa_{current_time}"
...
trainer = ContrastiveNERTrainer(
    ...
    contrastive_weight=0.0   # ✂️ 【消融实验1修改】：切断对比学习损失，权重设为 0.0
)

print(f"🚀 实验一启动：纯底座基线验证 (Baseline)。结果保存在: {output_dir}")
trainer.train()

print("📊 训练结束。正在【测试集】上进行纯底座基线最终评估...")
...
with open(os.path.join(output_dir, "evaluation_metrics.txt"), "w", encoding="utf-8") as f:
    f.write("========== Baseline Test Set Evaluation ==========\n")
...
print(f"🎉 实验一执行完毕！请查看控制台打印的 test_results，并记录 F1 分数！")