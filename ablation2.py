...
class MVCL_BERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, lexical_ids, structural_ids, labels=None):
        ...
        # ✂️ 【消融实验2】：直接跳过词法特征融合，保留原底座特征作为后续融合的基础
        fused_1 = inputs_embeds 
...
output_dir = f"./outputs/results_wo_Lexical_{current_time}"
...
trainer = ContrastiveNERTrainer(
    ...
    contrastive_weight=0.1  # 保留对比学习，仅做词法消融
)

print(f"🚀 消融实验二：去除词法视图 (w/o Lexical View)。结果保存在: {output_dir}")
trainer.train()

print("📊 训练结束。正在【测试集】上提取去除词法视图后的成绩...")
...
with open(os.path.join(output_dir, "evaluation_metrics.txt"), "w", encoding="utf-8") as f:
    f.write("========== w/o Lexical View Test Set Evaluation ==========\n")
...
print(f"🎉 实验二执行完毕！请查看控制台打印的 test_results，并记录 F1 分数！")